...
class ChromaDBHandler:
    ...
    def _initialize_collections(self):
        ...
        try:
            ...
            self.youtube_collection = self.client.get_or_create_collection(
                ...
                embedding_function=self.embedding_function,
                metadata={"description": "Collection for YouTube video transcriptions"}
            )
    ...
    def store_youtube_transcription(self, transcription, video_title: str,
                                    video_author: str, youtube_url: str, video_id: str,
                                    additional_metadata: Optional[Dict] = None):
        """
        Store YouTube video transcription in ChromaDB.

        Args:
            transcription (str or dict): Video transcription text or dict with 'text' key
            video_title (str): Title of the YouTube video
            video_author (str): Author/channel of the video
            youtube_url (str): URL of the YouTube video
            video_id (str): Unique ID of the YouTube video
        ...
        metadata = {
            ...
            "video_id": video_id,
            "timestamp": str(datetime.now()),
            **transcript_metadata  # Include any additional metadata from the transcription dict
        }
        ...
        # Generate a unique ID
        content_id = f"youtube_{uuid.uuid4()}"

        try:
            # Store in the YouTube collection
            ...
            logger.info(
                f"YouTube video stored successfully. Collection now has {self.youtube_collection.count()} items.")
            return content_id
        except Exception as e:
    ...
    def list_content(self, content_type="all"):
        ...
        Args:
            content_type (str): Type of content to list ("all", "blog", "podcast", "youtube")
        ...
        try:
            if content_type == "all":
                ...
                # Get YouTube content
                youtube_results = self._query_collection(self.youtube_collection)
                if youtube_results:
                ...
                return self._query_collection(self.youtube_collection)
    ...
    def get_youtube_videos(self):
        ...
        return self.list_content(content_type="youtube")
    ...
    def delete_content(self, content_id: str):
        ...
        collections_to_try = [self.blog_collection, self.podcast_collection, self.youtube_collection]
        content_found = False

        for collection in collections_to_try:
            try:
                collection.delete(ids=[content_id])
                logger.info(f"Content {content_id} deleted from {collection.name}")
                content_found = True
        ...
        # If we get here, the content wasn't found in any collection
    ...
    def get_context_by_type(self, query: str, content_type: str = "all", num_results: int = 5):
        ...
        Args:
            ...
            content_type (str): Type of content to search ("all", "blog", "podcast", "youtube")
        ...
        try:
            ...
            if content_type == "all" or content_type == "youtube":
                youtube_results = self._search_collection(
                    collection=self.youtube_collection,
                    query=query,
                    num_results=num_results
                )
    ...
    def get_content_by_id(self, content_id: str):
        ...
        # Try each collection
        collections = [self.blog_collection, self.podcast_collection, self.youtube_collection]
